...
* An `LLM_API_KEY` for your chosen provider (e.g. NVIDIA from https://build.nvidia.com/[build.nvidia.com]) for the production profile. Not needed for dev mode or local Docker, which use Ollama.
...
== 3. Production Docker (bring your own LLM + PostgreSQL)

Uses `docker-compose.yml`. Pick a provider with `LLM_KIND` (default `nvidia`) and set `LLM_API_KEY`.
