...
|*Schedule* (`/console/schedule`) |The five background jobs (memory review, memory curation, episodic consolidation, skill curation, skill lifecycle). Each row's effective cron and previous/next fire times come from the live Quarkus `Scheduler`, looked up by the explicit `@Scheduled(identity = ...)` each job declares, so the schedule shown always matches what is actually registered. The job list itself - label, cron property, trigger endpoint - is a static registry in the page: nothing in the framework links a job to the config property holding its cron. Editing a cron reuses the `RUN_TIME` runtime-toggle path; triggering a job now hits its manual `/api/admin/*` endpoint.
|===
